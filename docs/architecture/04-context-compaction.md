...
`context/compact.py:30`의 `_COMPACT_PROMPT_HEAD`는 LLM에게 다음 9개 항목을 요약 안에 보존하도록 지시한다:
...
```
<head: no-tools 가드 + 분석 지침 + 9개 섹션 명세>
...
<tail: Output format + REMINDER(도구 호출 금지)>
```

- **위치가 계약이다** — 슬롯은 9번 섹션 뒤·`Output format` 앞이다. 트레일링 `REMINDER`는 요약 턴이 도구 호출로 낭비되는 것을 recency로 막는 가드라, 주입 블록이 그 자리를 밀어내면 안 된다.
...
- **`tools=[]` 필수**: `compact_conversation()` 내 `provider.complete()` 호출은 반드시 `tools=[]`여야 한다(`context/compact.py:153`). 요약 중 도구 호출이 허용되면 tool_use↔tool_result 쌍 정합성이 깨질 수 있다.
- **`is_compact_summary=True` 플래그**: `create_compact_summary_message()`가 생성하는 user 메시지는 `is_compact_summary=True`로 표시된다(`context/compact.py:116`). 이 플래그는 [05-messages](05-messages.md)의 메시지 유형 분류와 연동되며, 제거하거나 누락하면 메시지 필터링 로직이 요약 메시지를 일반 사용자 메시지로 오분류할 수 있다.
...
- **`<summary>` 태그 폴백**: `<summary>` 태그가 없으면 전체 응답 텍스트를 그대로 사용한다(`context/compact.py:167-168`). LLM이 포맷을 어기더라도 루프가 중단되지 않도록 설계된 방어 코드다. 포맷 준수율이 낮아지면 요약 품질 저하로 이어지므로 프롬프트 수정 시 주의한다.
