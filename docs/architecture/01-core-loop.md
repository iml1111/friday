...
- **메모리 섹션 주입(빌트인).** `engine.step()`이 매 턴 `build_memory_section(self._memory)`를 조립(턴별 재구성, 캐시 없음)해 base 시스템 프롬프트 뒤에 덧붙인다. `compact()`는 이 경로를 거치지 않아 메모리 인덱스가 요약에 새지 않는다. `MemoryStore`는 `LoopState`에 직렬화되지 않으므로(컨테이너-로컬 재주입) 분산 재개 serde는 불변이다. 기본 store는 `FileMemoryStore`이며 `FridayAgent(..., memory=...)`로 교체한다. 상세는 [08-memory](08-memory.md) 참조.
