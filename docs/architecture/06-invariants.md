...
| 병렬 실행해도 결과는 `tool_use` 블록 원본 순서 | 쌍 매칭·재현성 파괴 | `tools/orchestrator.py:145` `run_tools()` — `asyncio.gather`는 인자 순서대로 결과를 반환하므로 완료 순서와 무관하게 블록 순서가 보존됨 → [02-tool-orchestration](02-tool-orchestration.md) |
| `tool_result` 메시지 `role="user"`, 첫 메시지 `user`, user/assistant 교대 | role 규칙 위반 시 API 거부 | `messages/types.py:40·54` — `create_tool_result_message()`·`create_user_message()`가 `role="user"`로 생성; `messages/normalize.py` — API 직렬화 시 role 태그 그대로 전달 → [05-messages](05-messages.md) |
| `step()`은 `state.messages` 전체를 API에 전송(윈도우 관리=호출자) | 임의 절사 시 컨텍스트 손실 | `core/loop.py:163-165` — `messages_for_query = list(state_messages)` 전체 전달; 오버플로는 `ContextOverflowError`로 호출자에게 전파 → `engine.compact()` 재시도 → [01-core-loop](01-core-loop.md)·[04-context-compaction](04-context-compaction.md) |
...
- **[03-llm-providers](03-llm-providers.md)** — `_build_params()` thinking/temperature 상호 배타, 빈 tools 처리
