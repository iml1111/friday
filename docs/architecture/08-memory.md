...
세션 경계를 넘어 타입화된 fact(user/feedback/project/reference)를 장기화한다. **opt-in**이다 — `FridayAgent(memory=None)`(기본)이면 서브시스템이 장착되지 않고(도구·지침·인덱스 리마인더 전부 없음), 호출자가 `MemoryStore`를 주입하면 그 store와 `tools()`가 통째로 장착된다. (설계 이력: 본래 always-on 빌트인이었으나 실전 실측 — 전 세션 도구 호출 0회, 매 콜 상시 토큰 지출 — 을 근거로 opt-in으로 전환. ai-sourcer 발산 리포트 §5.6 참조.)
