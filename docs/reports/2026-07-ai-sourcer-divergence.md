...
| 5 | 도구 스펙 다이어트 4종 + TodoWrite 적용 | §5.4 | ✅ **그대로 이식** | LLM-agnostic·의미 불변·하위호환·−17% 실측·테스트 동반 |
| 6 | 조립 순서 범용→구체 | §5.5 | ✅ **이식 권장** | 원칙 범용 + 실전 근거. 단 모델 응답 성향이 변하므로 의식적 결정 + docs 갱신 |
| 7 | 콜론 조항 보완 ("자명하면 무발화") | §5.5 | ✅ **이식** | 소형·독립·범용 |
| 8 | MEMORY_INSTRUCTIONS 압축 | §5.1 | ✅ **#2와 함께 이식** | 내용이 turn-local reminder 구조를 전제 — 단독 이식 불가 |
