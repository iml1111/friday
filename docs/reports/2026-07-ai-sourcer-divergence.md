...
>
> ↩️ **적용 후 제거** — 2026-07-22 `26b0ced`로 적용했다가 같은 날 `8cf1bae`로 제거. 사유: 컨텍스트 주입 인터페이스 단순화 — 정적은 `system_prompt` 문자열 조합으로 결과 동일, 동적 훅은 실소비자 0(YAGNI, 필요 시 무해하게 재도입 가능). 내부 turn-local 리마인더 기계(#3)는 유지
...
>
> ✅ **업스트림 적용 완료** — 2026-07-22, friday `cd59c3d` (논의 후 opt-in으로 결정. 헌장·docs 00/01/08·CLAUDE.md 개정, run_agent는 명시 장착으로 유지. 이로써 vendored 발산 기록의 유일 항목 소멸)
...
| 4 | `system_sections` · `turn_sections` 훅 | §5.3 | ↩️ **적용 후 제거** (`26b0ced` → `8cf1bae`) | 검토 결과 주입 인터페이스 단순화 우선 — 정적은 `system_prompt` 조합으로 충분, 동적 훅은 실소비자 0(YAGNI). 내부 기계(`turn_reminders`)는 유지 |
...
| 9 | 메모리 opt-in 전환 | §5.6 | ✅ **적용 완료** (2026-07-22, `cd59c3d`, 논의 후 결정) | 실측 근거(호출 0회·~1k tok/콜) + 도메인-무관 SDK 정합성으로 opt-in 채택. breaking — 헌장·docs 개정 동반 |
